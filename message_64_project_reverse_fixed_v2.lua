    ...
    for Name, Value in pairs(Node) do
        ...
        elseif type(Value) == "table" then
            ...
            elseif Rarity == nil then
                -- Do not use table.find here. Some executor Lua runtimes
                -- implement table.find differently and can throw an
                -- "argument #3 ... index out of range" error.
                local Candidate = tostring(Name)
                local CandidateLower = Candidate:lower()

                for _, KnownRarity in ipairs(RarityPriority) do
                    if Candidate == KnownRarity
                        or CandidateLower == KnownRarity:lower() then
                        Rarity = KnownRarity
                        break
                    end
                end
...
-- Lookup diagnostic:
-- Set this to an inventory item name when testing a specific value.
local DEBUG_LOOKUP_NAME = ""

if DEBUG_LOOKUP_NAME ~= "" then
    local DebugInfo = FindItemValue(DEBUG_LOOKUP_NAME)

    if DebugInfo then
        print(string.format(
            "[MM2] LOOKUP OK: %s | value=%s | rarity=%s | chroma=%s",
            DEBUG_LOOKUP_NAME,
            tostring(DebugInfo.Value),
            tostring(DebugInfo.Rarity),
            tostring(DebugInfo.Chroma)
        ))
    else
        warn("[MM2] LOOKUP MISS:", DEBUG_LOOKUP_NAME)
    end
end
