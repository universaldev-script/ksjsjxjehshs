...
local function LoadDatabase()
    ...
    if not Response or type(Response.Body) ~= "string" or #Response.Body < 2 then
    ...
    -- Prefer JSON. This is the safest format for a data-only database.
    local JSONOK, JSONResult = pcall(function()
        return HttpService:JSONDecode(Body)
    end)

    if JSONOK and type(JSONResult) == "table" then
        RawDatabase = JSONResult
    else
        -- Backward compatibility with the existing database endpoint, which
        -- returns a Lua table expression rather than JSON.
        ...
        if LoaderOK and type(LoaderResult) == "table" then
            RawDatabase = LoaderResult
        else
            warn("[MM2] Could not decode the item database as JSON or a Lua table.")
            return nil, nil
        end
    end
    ...
    local function AddItemMapping(ItemName, RarityName, Value)
        if type(ItemName) ~= "string" or ItemName == "" or type(Value) ~= "number" then
            return
        end

        local SafeName = ItemName
        local LowerName = string.lower(SafeName)

        -- Normalize common MM2 naming differences:
        -- DefaultGun, Default Gun, default_gun and default-gun all resolve
        -- to the same lookup key.
        local Key1 = LowerName:gsub("[^%w]", "")
        local Key2 = LowerName:gsub("[^%s]", "")
        local Key3 = LowerName:gsub("[%s_%-]", "")
        local Key4 = LowerName:gsub("'", ""):gsub(" ", "")

        local ItemInfo = {
            Rarity = tostring(RarityName or "Unknown"),
            Value = Value,
            Chroma = (LowerName ~= "" and string.lower(tostring(RarityName)) == "chroma")
        }
    ...
    for RarityName, ItemsList in pairs(RawDatabase) do
        if type(ItemsList) == "table" then
            FoundRarities[tostring(RarityName)] = true

            for ItemName, Value in pairs(ItemsList) do
                -- Normal database format: Rarity -> ItemName -> number
                if type(Value) == "number" then
                    AddItemMapping(ItemName, tostring(RarityName), Value)

                -- Also accept ItemName -> {Value=..., Rarity=...} style data.
                elseif type(Value) == "table" then
                    local ItemValue = tonumber(Value.Value or Value.value or Value.Price or Value.price)
                    local ItemRarity = Value.Rarity or Value.rarity or RarityName

                    if ItemValue then
                        AddItemMapping(ItemName, tostring(ItemRarity), ItemValue)
                        FoundRarities[tostring(ItemRarity)] = true
                    end
                end
            end
        end
    ...
    if next(Mapping) == nil then
        warn("[MM2] Database decoded successfully, but contained no numeric item values.")
        return nil, nil
    end

    warn("[MM2] Loaded item database successfully.")
        ...
        for ItemId, Quantity in pairs(ProfileData.Weapons.Owned) do
            if Quantity and Quantity > 0 then
                ...
                if SafeName ~= "" then
                    ...
                    local Key3 = LowerName:gsub("[%s_%-]", "")
                    local Key4 = LowerName:gsub("'", ""):gsub(" ", "")
                    local ItemInfo = ItemDatabase[Key1]
                        or ItemDatabase[Key2]
                        or ItemDatabase[Key3]
                        or ItemDatabase[Key4]
